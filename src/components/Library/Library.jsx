...
function Library({ editArticle }) {
  ...
  const getSubcategories = (index) => {
    ...
    return cats.subCategories[selectedCategory] || [];
  };
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <Button
                ...
                onClick={(event) =>
                  handleOptionButtonClick(event, index, optionIndex)
                }
              >
                Btn
              </Button>
  ...
  const handleUpdateArticle = async (id) => {
    try {
      ...
      return api.open({
        message: "Congrats your Article updated correctly 🤩",
        description: `Your Market Article updated successfully`,
        icon: <SmileOutlined style={{ color: "#108ee9" }} fill="#108ee9" />,
      });
    } catch (error) {
      console.log(error);
  ...
  const handleDeleteArticle = async (id) => {
    ...
    await article.deleteArticlesById(id);
    setArticles((prevState) =>
